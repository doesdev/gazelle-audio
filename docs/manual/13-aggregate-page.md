...
Each card lists the interface's output sockets, in the same order, and which of its USB playback channels reach each one: **Monitor: USB 1 PLAY 1 to 2, through Mix 1**, **S/PDIF out: USB 1 PLAY 5, directly**, or both, one after the other. A pair of sockets, left and right, is one line; a larger group is a line per socket, so the Studio+'s eight line outs and sixteen ADAT outs are **Line out 1**, **Line out 2** and so on. It is the quickest way to see that something the DAW plays will actually be heard somewhere, and where.
