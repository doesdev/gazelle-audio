...
- **Control Room Mono** sums a mix to mono by centring its pans, so every output that mix feeds goes mono, including a recording output. The button's tooltip names them. It does not jump the level: centring both sides is about 6 dB louder, so Gazelle lowers that mix by 6 dB while mono is on and gives that step back when it ends.
