...
Gazelle has been used on **one person's two interfaces**: one Zen Quadro Synergy Core and one Zen Studio+, on one Windows 11 PC. It has not been tried on any other unit, any other firmware, any other computer or any other operating system. It is new software, at version 1.4.0.
...
The manual describes Gazelle as the code stands at version 1.4.0 on Windows. The screenshots were taken from the running app against its built-in emulator (`--backend loopback`), so the device names are made up and the meters show a test pattern.
