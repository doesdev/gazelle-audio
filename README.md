...
|---|---|---|
| `antelope-protocol` | The byte-level wire protocol: 16-byte header, field grammar, payload serialization, CRC32. Transport-agnostic. | 22 |
| `antelope-transport` | Framing, segmentation, request/response correlation, and a hardware-free loopback device. | 25 |
| `antelope-server` | Multi-device HTTP + WebSocket control server. Loopback-backed; no hardware adapter yet. | 31 |
