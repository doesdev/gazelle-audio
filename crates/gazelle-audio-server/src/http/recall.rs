...
//!   then it only reports the bytes, because sending them waits for the hardware session (spec §6,
...
pub async fn apply_recall(
    ...
    if !state.force_dry_run {
        ...
        // it is missing on purpose: the hardware session in the spec's §6 has to confirm the order,
        // the hard mute's behaviour and what a preset recall changes first (decision 0012).
        return Err(ServerError::Unsupported(
            "applying a recall to a device is not built: the plan above is complete, and sending it waits for the hardware session in the workspace spec §6. Run the server with --dry-run to see the bytes each step would send".into(),
        ));
    }
