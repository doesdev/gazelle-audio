//! Integration test: load the real recovered `quadro_commands.json` and
...
/// The cyclic layout carried in the extracted schema must decode the 0x73 report to the
/// same values as the layout built independently from the panel's report format.
///
/// Two paths to the same answer: `cyclic_gt.json` is generated by `gen_cyclic_gt.py`
/// straight from `zenstudiotb_report_format.py`, while the registry's layout comes from
/// `extract_field_layouts.py`. Agreement means the extractor did not distort the layout on
/// its way into the schema — which matters, because an earlier version of that extractor
/// silently dropped every sub-byte field's bit width.
#[test]
fn schema_cyclic_layout_matches_independent_ground_truth() {
    let (report, gt) = load_cyclic_ground_truth();
    ...
    let layout = reg
        .cyclic(0x73)
        .expect("the schema must carry a layout for the 0x73 cyclic report");
    assert_eq!(layout.report_id, 0x73);

    let decoded = layout
        .parse_contents(&report[16..])
        .expect("decode 0x73 via the schema layout");

    assert_values_match(&decoded, &gt["expected"], "0x73 (schema layout)");
}

/// Every device schema declares the cyclic reports the event stream depends on.
#[test]
fn registries_declare_their_cyclic_reports() {
    let reg = load_registry();
    assert!(reg.cyclic_len() >= 2, "expected at least the 0x73 and 0x83 layouts");
    assert!(reg.cyclic(0x73).is_some(), "0x73 is the main state report");
    assert!(reg.cyclic(0x83).is_some(), "0x83 carries meter data");
    // 0x73 carries the bulk of device state.
    assert!(
        reg.cyclic(0x73).unwrap().fields.len() > 50,
