...
//! tokio, so `antelope-protocol` and `antelope-transport` stay runtime-free.
//!
...
use antelope_protocol::registry::Registry;
use antelope_protocol::wire::Header;
use antelope_transport::correlation::{Correlation, ResponseCorrelator};
use antelope_transport::{segment_report, Device, RawPacket, Report};
...
fn handle_request(
    ...
    let segments = segment_report(
        ...
        header.ext2,
        header.ext3,
        &bytes[antelope_protocol::wire::HEADER_SIZE..],
    ...
    for seg in &segments {
        let report = Report {
            ...
            contents: seg[antelope_protocol::wire::HEADER_SIZE..].to_vec(),
        };
        ctx.device
            .send(&report)
...
fn decode_returns(
    command: &antelope_protocol::Command,
    report: &Report,
) -> Result<HashMap<String, Value>, String> {
    let mut buf = Vec::with_capacity(antelope_protocol::wire::HEADER_SIZE + report.contents.len());
