...
use antelope_server::device::descriptor::DeviceId;
use antelope_server::device::manager::{DeviceManager, ServerEvent};
use antelope_server::device::worker::DeviceEvent;
use antelope_server::registry_set::{RegistrySet, PID_QUADRO};
use antelope_transport::{Device, RawPacket, Report};
...
impl Device for CannedDevice {
    fn send(&mut self, _report: &Report) -> Result<bool, antelope_protocol::wire::WireError> {
        Ok(true)
    }
...
async fn cyclic_reports_arrive_decoded() {
    ...
    let size: usize = layout.fields.iter().map(antelope_protocol::field::Field::size).sum();
