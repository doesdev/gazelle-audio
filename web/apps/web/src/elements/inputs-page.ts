...
/** The styles of a preamp card and a digital input cell, for any element that shows them. */
export const INPUT_CONTROL_STYLES = `
  .preamp {
    display: grid;
    grid-column: span 2;
    gap: 6px;
...
/** Where a control built outside an element keeps its effects: the element's own `watch` and `onDisconnect`. */
...
export class GaInputs extends GaElement {
  static override styles = [
    sheet(`
      ...
      .mic { --ga-field-height: 24px; display: grid; grid-template-columns: minmax(64px, 88px) minmax(0, 1fr) minmax(0, 2fr) auto auto; align-items: center; gap: 8px; padding: 6px 8px; border-radius: 3px; background: var(--ga-surface-raised); }
      .mic select { min-width: 0; }
...
/**
 * One preamp's card: type, gain, 48V behind its confirming click, phase invert and the HPF the device
 * reports, with its link badge unless `links` is false (a page without the link bar).
 */
export function preampCard(host: ControlHost, inputs: InputsModel, i: number, enabled: () => boolean, { links = true }: { links?: boolean } = {}): HTMLElement {
...
/** One digital input's cell: its gain, editable where the device's own panel sets it, and its link badge unless `links` is false. */
export function digitalCell(host: ControlHost, inputs: InputsModel, group: DigitalGroup, i: number, enabled: () => boolean, { links = true }: { links?: boolean } = {}): HTMLElement {
  ...
  const link = group.editable && links ? linkButton((fn) => host.watch(fn), group.kind, inputs.deviceId, i, `${group.kind}-link-${i}`) : undefined;
