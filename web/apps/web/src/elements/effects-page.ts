...
import { effect as effectOf, untracked } from "../core/signal.ts";
import { formatReverbLevel, formatRoomSize, REVERB_LEVEL_MAX, REVERB_LEVEL_MIN, REVERB_LEVEL_UNITY, REVERB_RETURN_MAX, REVERB_SEND_MAX, type EffectChain, type EffectsModel } from "../store/effects.ts";
import { formatPan, PAN_CENTRE, PAN_MAX, PAN_MIN } from "../store/mixer.ts";
...
export class GaEffects extends GaElement {
  ...
  protected override render(): void {
    ...
    this.watch(() => {
      ...
      chains.replaceChildren(
        ...Array.from({ length: count }, (_, index) => this.#chain(effects, index, list?.[index], deviceId, destination, disposers)),
      );
    ...
    this.root.replaceChildren(
      ...
      note,
      h("section", {}, h("h2", {}, "Effect chains"), h("p", { class: "note" }, "Each chain processes what routing sends to its AFX IN channel and returns on AFX OUT. Bypass is per effect; the device does not report it here, so neither button shows lit until you set one. Adding, moving and removing effects, and their settings, are not in this version."), chains),
      reverb,
      ...returnsAndSends,
    );
  ...
  #chain(effects: EffectsModel, index: number, chain: EffectChain | undefined, deviceId: string, destination: number, disposers: (() => void)[]): HTMLElement {
    const store = useStore();
    const topology = store.topology(deviceId);
    ...
    const rows = chain.slots.map((slot) => {
      ...
      return h(
        ...
        h("span", { class: "effect", title: `Effect type ${slot.type}, instance ${slot.inst + 1}` }, slot.name, " ", h("span", { class: "instance" }, `#${slot.inst + 1}`)),
