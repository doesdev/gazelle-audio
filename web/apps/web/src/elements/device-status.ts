...
function confirmedChoice(select: HTMLSelectElement, testId: string, describe: (index: number) => string, send: (index: number) => void) {
  ...
  const confirm = h("button", { type: "button", class: "confirm", "data-testid": testId, hidden: true }, "Confirm");
  const choice = {
  ...
  protected override render(): void {
    ...
    if (device.family !== null) {
      ...
      const buttons = slots.map((slot) =>
        h("button", { type: "button", "data-testid": `preset-${slot}`, "aria-label": `Recall preset ${slot}`, title: `Recall preset ${slot}: click twice` }, String(slot)),
      );
    ...
    if (clock !== undefined) {
      const source = h(
        "select",
        { "aria-label": "Clock source", "data-testid": "clock-source", "data-no-wheel": true },
        clock.sources.map((name, index) => h("option", { value: String(index) }, name)),
      );
      const rate = h(
        "select",
        { "aria-label": "Sample rate", "data-testid": "clock-rate", "data-no-wheel": true },
