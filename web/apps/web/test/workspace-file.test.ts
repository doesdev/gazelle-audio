...
test("a read file is summarised for the confirmation, with every device it names", () => {
  ...
  assert.equal(read.ok, true);
  if (!read.ok) return;
  assert.deepEqual(read.summary, { names: 2, groups: 2, links: 1, mixers: 1, layouts: 1, surfaces: 1, devices: ["loopback-0", "loopback-1", "serial-3", "serial-5", "serial-7", "serial-9"] });
});
