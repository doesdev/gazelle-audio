...
const SEED = {
  ...
  control_room: { "loopback-0": { outputs: [0, 3] } },
};
...
test("an exported workspace imports back exactly, after confirming, without touching a device", async ({ page }) => {
  ...
  await expect(confirm).toContainText("2 device names, 1 group, 1 link, 1 mixer layout, 1 saved layout, 1 surface, 1 cable and 1 Control Room choice");
  expect(await serverWorkspace(), "nothing is replaced before confirming").toEqual({ version: 1, groups: [], links: [], aliases: {}, mixers: {}, layouts: [], device_colors: {}, surfaces: [], cables: [], control_room: {} });
