...
import { byteSize, parseField, SCALAR_BYTES, type Field, type Scalar } from "./field.ts";
...
function familyFile(input: SchemaInput): string {
  ...
  out.push(`import type { Bytes, FamilySchema } from "../schema.ts";`, "");
...
function indexFile(inputs: readonly SchemaInput[]): string {
  ...
  for (const { family } of inputs) {
    const P = pascal(family);
    out.push(`import { ${family}Schema, type ${P}Commands, type ${P}CyclicReports } from "./${family}.ts";`);
  }
